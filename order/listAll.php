            ...
            if ($result = mysqli_query($con, $sql)) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    foreach ($str as $value) {
                        $prodInfo = explode(',', $value);

                        $prodQuantity = $prodInfo[0];
                        $prodSize = isset($prodInfo[1]) ? $prodInfo[1] : null;
                        $prodId = isset($prodInfo[2]) ? $prodInfo[2] : null;

                        $sqll = "SELECT * FROM products WHERE `id` = '{$prodId}'";

                        if ($resultt = mysqli_query($con, $sqll)) {
                            while ($roww = mysqli_fetch_assoc($resultt)) {
                                $products[$i]['quantity'] = $prodQuantity;
                                $products[$i]['chosenSize'] = $prodSize;
                                ...
                                $products[$i]['picUrl'] = 'http://localhost/ClothesShopApi/product/images/' . $roww['picName'];
                            }
                        } else {
