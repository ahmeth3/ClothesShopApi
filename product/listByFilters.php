...
if (isset($postdata) && !empty($postdata)) {
    ...
    */
    if ($category === 'View All' && $color === 'None' && $size === 'None') //000
        $sql = "SELECT * FROM products WHERE `gender` = '{$gender}'";
    elseif ($category === 'View All' && $color === 'None' && $size != 'None') //001
        $sql = "SELECT * FROM products WHERE `gender` = '{$gender}' AND `size` LIKE '%$size%'";
    ...
    elseif ($category === 'View All' && $color != 'None' && $size != 'None') //011
        $sql = "SELECT * FROM products WHERE `gender` = '{$gender}' AND `color` LIKE '%$color%' AND `size` LIKE '%$size%'";
    ...
    elseif ($category != 'View All' && $color === 'None' && $size != 'None') //101
        $sql = "SELECT * FROM products WHERE `gender` = '{$gender}' AND `category` = '{$category}' AND `size` LIKE '%$size%'";
    elseif ($category != 'View All' && $color != 'None' && $size === 'None') //110
        ...
        $sql = "SELECT * FROM products WHERE `gender` = '{$gender}' AND `category` = '{$category}' AND `color` LIKE '%$color%' AND `size` LIKE '%$size%'";
    }
