...
if (isset($postdata) && !empty($postdata)) {
    ...
    if ($jwt) {
        ...
        try {
            ...
            // Sanitize.
            $sellerId = mysqli_real_escape_string($con, trim($request->data->sellerId));
            ...
            // Store.
            $sql = "INSERT INTO `userorders`(`buyerId`,`sellerId`,`status`,`price`, `date`, `orderedProducts`,`name`, `surname`, `phone`, `address`, `city`, `zipCode`, `country`) VALUES ('{$id}','{$sellerId}','{$status}','{$price}','{$date}','{$orderedProducts}','{$name}','{$surname}','{$phone}','{$address}','{$city}','{$zipCode}','{$country}')";
